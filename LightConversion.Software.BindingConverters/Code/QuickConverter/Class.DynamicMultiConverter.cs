...
namespace LightConversion.Software.BindingConverters.QuickConverter {
    public class DynamicMultiConverter : IMultiValueConverter {
        ...
        private readonly Func<object[], object[], object> _converter;
        ...
        public object Convert(object[] values, Type targetType, object parameter, System.Globalization.CultureInfo culture) {
            for (int i = 0; i < values.Length; ++i) {
                if (values[i] == DependencyProperty.UnsetValue || _namedObjectType.IsInstanceOfType(values[i]) || (PTypes[i] != null && !PTypes[i].IsInstanceOfType(values[i]))) return DependencyProperty.UnsetValue;
            }
            ...
            try {
                ...
                LastException = e;
                ++ExceptionCount;
                if (Debugger.IsAttached) Console.WriteLine("QuickMultiConverter Exception (\"" + ConvertExpression + "\") - " + e.Message + (e.InnerException != null ? " (Inner - " + e.InnerException.Message + ")" : ""));
                ...
                if (_toDataContainers != null) {
                    foreach (var container in _toDataContainers) container.Value = null;
                }
            }
        ...
        public object[] ConvertBack(object value, Type[] targetTypes, object parameter, System.Globalization.CultureInfo culture) {
            if (_chainedConverter != null) {
                ...
                if (value == DependencyProperty.UnsetValue || value == System.Windows.Data.Binding.DoNothing) return new object[targetTypes.Length].Select(o => value).ToArray();
            }

            object[] ret = new object[_convertBack.Length];

            if (ValueType != null && !ValueType.IsInstanceOfType(value)) {
                for (int i = 0; i < ret.Length; ++i) ret[i] = System.Windows.Data.Binding.DoNothing;
                return ret;
            }
            ...
            for (int i = 0; i < _convertBack.Length; ++i) {
                try {
                    ...
                    LastException = e;
                    ++ExceptionCount;
                    if (Debugger.IsAttached) Console.WriteLine("QuickMultiConverter Exception (\"" + ConvertBackExpression[i] + "\") - " + e.Message + (e.InnerException != null ? " (Inner - " + e.InnerException.Message + ")" : ""));
            ...
            if (_fromDataContainers != null) {
                foreach (var container in _fromDataContainers) container.Value = null;
            }
            return ret;
        }

        private object CastResult(object result, Type targetType) {
            if (result == null || result == DependencyProperty.UnsetValue || result == System.Windows.Data.Binding.DoNothing || targetType == null || targetType == typeof(object)) return result;
