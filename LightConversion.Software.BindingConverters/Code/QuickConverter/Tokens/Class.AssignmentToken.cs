...
namespace LightConversion.Software.BindingConverters.QuickConverter.Tokens {
    public class AssignmentToken : TokenBase, IPostToken {
        ...
        public override Type ReturnType {
            get { return Value.ReturnType; }
        }

        public override TokenBase[] Children {
            get { return new[] { Target, Value }; }
        }
        ...
        internal override bool SetPostTarget(TokenBase target) {
            if (target is StaticMemberToken) {
                if ((target as StaticMemberToken).Member is PropertyInfo) {
                    if (!((target as StaticMemberToken).Member as PropertyInfo).CanWrite) throw new Exception("Static member \"" + ((target as StaticMemberToken).Member as PropertyInfo).Name + "\" is readonly and cannot be set.");
                    else if (Operator != default(Operator) && !((target as StaticMemberToken).Member as PropertyInfo).CanRead) throw new Exception("Static member \"" + ((target as StaticMemberToken).Member as PropertyInfo).Name + "\" is writeonly and cannot be read.");
                } else if (!((target as StaticMemberToken).Member is FieldInfo)) return false;
            } else if (!(target is InstanceMemberToken)) return false;
            Target = target;
            if (Operator != default(Operator)) Value = new BinaryOperatorToken(Target, Value, Operator);
            return true;
        }
        ...
        internal override bool TryGetToken(ref string text, out TokenBase token, bool requireReturnValue = true) {
            ...
            if (temp[0] != '=') {
                if (temp[1] != '=' || temp.Length < 3) return false;
                for (int i = (int)Operator.Multiply; i <= (int)Operator.Subtract; ++i) {
                    if (EquationTokenizer.Representations[i][0] == temp[0]) op = (Operator)i;
                }
                for (int i = (int)Operator.BitwiseAnd; i <= (int)Operator.BitwiseXor; ++i) {
                    if (EquationTokenizer.Representations[i][0] == temp[0]) op = (Operator)i;
                }
                if (op == default(Operator)) return false;
            }

            if (op == default(Operator)) temp = temp.Substring(1).TrimStart();
            else temp = temp.Substring(2).TrimStart();
            TokenBase valToken;
            if (!EquationTokenizer.TryEvaluateExpression(temp, out valToken)) return false;
            text = "";
