...
namespace LightConversion.Software.BindingConverters.QuickConverter.Tokens {
    public class NullPropagatingToken : TokenBase, IPostToken {
        ...
        internal override bool TryGetToken(ref string text, out TokenBase token, bool requireReturnValue = true) {
            token = null;
            string temp = text;
            if (temp.Length < 2 || temp[0] != '?' || (temp[1] != '.' && temp[1] != '[')) return false;
        ...
        internal override Expression GetExpression(List<ParameterExpression> parameters, Dictionary<string, ConstantExpression> locals, List<DataContainer> dataContainers, Type dynamicContext, LabelTarget label, bool requiresReturnValue = true) {
            ...
            return Expression.Block(new Expression[] { Expression.Assign(Expression.Property(constant, "Value"), Target.GetExpression(parameters, locals, dataContainers, dynamicContext, label)), Expression.IfThen(Expression.Equal(Expression.Property(constant, "Value"), Expression.Default(typeof(object))), Expression.Goto(label, Expression.Default(typeof(object)))), Expression.Property(constant, "Value") });
        }
    }
}