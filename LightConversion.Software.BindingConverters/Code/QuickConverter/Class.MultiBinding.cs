...
namespace LightConversion.Software.BindingConverters.QuickConverter {
    ...
    public class MultiBinding : MarkupExtension {
        ...
        public override object ProvideValue(IServiceProvider serviceProvider) {
            try {
                bool getExpression;
                if (serviceProvider == null) getExpression = false;
                else {
                    var targetProvider = serviceProvider.GetService(typeof(IProvideValueTarget)) as IProvideValueTarget;
                    if (targetProvider != null && (targetProvider.TargetObject is Setter)) getExpression = false;
                    else if (targetProvider == null || !(targetProvider.TargetProperty is PropertyInfo)) getExpression = true;
                    else {
                        Type propType = (targetProvider.TargetProperty as PropertyInfo).PropertyType;
                        if (propType == typeof(MultiBinding)) return this;
                ...
                if (FallbackValue != null) holder.FallbackValue = FallbackValue;
                if (TargetNullValue != null) holder.TargetNullValue = TargetNullValue;
                ...
                if (ExternalConverter == null) {
                    ...
                    foreach (string name in parameterOrder) holder.Bindings.Add(typeof(MultiBinding).GetProperty(name).GetValue(this, null) as BindingBase);

                    if (!_modeSet) {
                        bool exists = false;
                        for (int i = 0; i <= 9; ++i) exists |= !String.IsNullOrWhiteSpace(typeof(MultiBinding).GetProperty("ConvertBack" + i).GetValue(this, null) as string);
                        holder.Mode = exists ? BindingMode.TwoWay : BindingMode.OneWay;
                    }
                } else {
                    if (ExternalConverterParameterMode == ConverterParameterMode.Parameter) holder.ConverterParameter = ConverterParameter;
                    else holder.ConverterParameter = new object[] { V0, V1, V2, V3, V4, V5, V6, V7, V8, V9 };
                    holder.Converter = ExternalConverter;
                    for (int i = 0; i <= 9; ++i) {
                        var binding = typeof(MultiBinding).GetProperty("P" + i).GetValue(this, null) as BindingBase;
                        if (binding != null) holder.Bindings.Add(binding);
                    }
                }
